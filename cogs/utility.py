...
class Utility(commands.Cog):
    ...
    async def about(self, ctx):
        ...
        desc = "Il s'agit d'un bot Discord open source qui sert de moyen pour "
        desc += "membres pour communiquer facilement avec les administrateurs de serveur "
        desc += "d'une manière organisée."
        embed.description = desc

        embed.add_field(name="Uptime", value=self.bot.uptime)
        embed.add_field(name="Latency", value=f"{self.bot.latency * 1000:.2f} ms")
        embed.add_field(name="Version", value=f"`{self.bot.version}`")
        embed.add_field(name="Authors", value="`Arnaud`")
